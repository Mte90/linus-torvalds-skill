...
from torvalds_skill.distill import _run_categories_parallel, _distill_category
...
class TestDistillCategory:
    ...
    def test_non_empty_patterns_calls_llm(self):
        ...
        patterns = [
            {"trigger": "test trigger", "principle": "test principle", 
             "severity": "reject", "source": "email", "quote": "test quote"}
        ]
...
class TestDistillSkillIntegration:
    ...
    def test_distill_skill_uses_parallel_for_glm52(self):
        ...
        patterns = [
            {"category": "cat1", "trigger": "t1", "principle": "p1", "severity": "reject", "source": "email", "quote": "q1"},
            {"category": "cat2", "trigger": "t2", "principle": "p2", "severity": "reject", "source": "email", "quote": "q2"},
        ]
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('torvalds_skill.distill._call_llm') as mock_call_llm:
                ...
                with patch('torvalds_skill.distill._load_interview_data') as mock_interview:
                    ...
                    with patch('torvalds_skill.distill._load_interlocutor_variation_data') as mock_iv:
                        ...
                        with patch('torvalds_skill.distill._load_json_cached') as mock_load:
                            ...
                            with patch('torvalds_skill.distill.config.WALL_CLOCK_CATEGORY', 300):
                                with patch('torvalds_skill.distill.log_decision'):
                                    ...
                                    assert mock_call_llm.call_count >= 3  # 2 categories + 1 synthesis
