...
- **Bugfix (review):** Decontaminated the Qwen review — `llm_review.py` now strips `reasoning_parts` from the response before cache write (lines 137-164), and a `_validate_review_format` gate (lines 172-177) rejects any review containing chain-of-thought markers before it enters the cache. Cleared four contaminated entries from `data/unified_cache.jsonl`. Re-ran the Qwen baseline review clean (6 265 bytes, 873 words, zero CoT markers) and rebuilt `report/comparison.md` — all four models now have consistent declared=parsed finding counts (qwen 18/18, gpt-oss 15/15, glm 8/8, mistral 50/50).
- **Bugfix (review):** Raised `review_max_tokens` to 131 072 for qwen3.8-27b (240K context) — the previous 32 000 budget was consumed entirely by the reasoning phase, producing `reasoning_only_response` with zero review content. Added a `disable_thinking` fallback in `llm_review.py` that auto-retries with `chat_template_kwargs={"enable_thinking": false}` when a review ends with `reasoning_only_response`, capped at one retry per model.
- **Bugfix (pipeline):** Added missing `import os` in `scripts/run_pipeline.py` — `os.chdir` raised `NameError` on every orchestrator dry-run.
- **Bugfix (profiles):** Fixed env-var override key generation in `profiles.py:200` — `model_name.upper().replace("-", "_")` did not replace dots, so `glm5.2` produced `GLM5.2` (not `GLM5_2`) and `qwen3.8-27b` produced `QWEN3.8-27B` (not `QWEN3_8_27B`), making every `LLM_PROFILE_*` env override silently ineffective for dotted model names. Added `.replace(".", "_")`. Updated stale `GLM52` examples in `README.md`, `AGENTS.md`, `.env.example` to the correct `GLM5_2` / `QWEN3_8_27B` forms.
- **Bugfix (triggers):** Fixed `detect_style()` routing in `verify_skill.py` — the regex only matched plain `### Level` headings, so Mistral files (which use bold-wrapped `### **Level N: Title**`) fell through to the gpt-oss extractor and all triggers landed in "General". Expanded the regex to match both plain and bold-wrapped heading variants. Tightened `extract_triggers_mistral` to match the exact `**Trigger**:` pattern (was matching any bold token as a title, pulling in `**Correctness**` / `**Performance**` bullets). Fixed `in_level` flag reset to clear only on exact `###` headings, not on every `####`. Mistral trigger count corrected from 18 to 31.
- **Feature (verify):** Added `check_mode_profile_consistency()` cross-check in `verify_skill.py` — verifies that the `mode` field in a skill file's frontmatter matches the model's profile `distill_mode`. All four skills now pass (single-call for all reasoning models).
- **Feature (triggers):** Early triggers appearing before the first theme heading are now assigned to the first detected theme instead of falling back to "General".
- **Cleanup (docs):** Replaced dead cache env vars (`LLM_CACHE_PATH`, `LLM_CACHE_TTL_HOURS=24`, `EXTRACT_CACHE`) in `.env.example`, `docs/pipeline.md`, `docs/models.md` with the live unified-cache vars (`CACHE_ENABLED`, `CACHE_PATH`, `CACHE_TTL_HOURS=168`).
- **Cleanup (docs):** Updated `docs/models.md`, `README.md`, `soul/README.md` with final regenerated word counts. Synced `comparison.md` distill-mode table to show `single-call (profile.reasoning)` for all four models.
- **Tests:** Updated stale trigger-routing expectations after skill regeneration (Mistral count 18→31, benchmark coverage 41/43→39/43). Full suite 1 014 passed.
